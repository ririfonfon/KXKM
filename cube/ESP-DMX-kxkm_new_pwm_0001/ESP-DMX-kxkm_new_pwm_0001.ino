...
const uint32_t period = 4 * 1024 ; // * 200ns ^= 1 kHz
...
void copyDMXToOutput(void) {
  ...
  pwm_set_duty(u8_to_12_gamma(artNetInterface->getSlot(adr), artNetInterface->getSlot(adr + 1)), 0);

  pwm_start();           // commit

  //analogWrite(14,(artNetInterface->getSlot(adr)*4 ));
  //analogWrite(14, u8_to_10_gamma(artNetInterface->getSlot(adr), artNetInterface->getSlot(adr+1)));
  //Serial.println ("valu :");
  //Serial.println((uint8_t)(artNetInterface->getSlot(adr)));
  //Serial.println((uint8_t)(artNetInterface->getSlot(adr+1)));
  //Serial.println (u8_to_10_gamma(artNetInterface->getSlot(adr), artNetInterface->getSlot(adr+1)));
}
